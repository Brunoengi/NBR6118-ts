...
interface IELSD extends IELS {
    type: PrestressingDesignType;
}

export class ELSD extends AbstractELS {
    ...
    constructor(inputs: IELSD) {
        super(inputs);
        this.type = inputs.type;

        if (this.type === 'Limited') {
            this.limit_tensions = {
                sigma1: { value: 0, unit: 'kN/cm²' },
                sigma2: { value: -0.45 * this.concrete.fck.value, unit: 'kN/cm²' }
            };
        ...
        // Verification for decompression (no tension)
        this.verification_sigma1 = this.verification_sigma({
            sigma: this.sigma1,
            limit: this.limit_tensions.sigma1,
            operator: (stressValue, limitValue) => stressValue <= limitValue // stress must be <= 0
        });
